...
bool osgi::removeDirRecursively (const QString & dirName)
{
    ...
    {
        foreach (QFileInfo info, dir.entryInfoList(QDir::NoDotAndDotDot | QDir::System | QDir::Hidden | QDir::AllDirs |
...
bool osgi::copyDirRecursively (const QString & srcPath,
...
// See http://stackoverflow.com/questions/2536524/copy-directory-using-qt
    ...
    {
        qCritical() << "osgi::copyDirRecursively: Failed to create destination directory" <<
            QFileInfo(dstPath).fileName();
    ...
    foreach(const QFileInfo &info, srcDir.entryInfoList(QDir::Dirs | QDir::Files | QDir::NoDotAndDotDot))
    {
        ...
        if( info.isDir())
        {
            ...
            {
                qCritical() << "osgi::copyDirRecursively: Failed to copy files from " << srcItemPath <<
                    " into " <<
                    dstItemPath;
