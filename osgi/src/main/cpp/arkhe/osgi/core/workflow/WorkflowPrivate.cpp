...
osgi::core::WorkflowPrivate::WorkflowPrivate(Workflow& object)
	...
	, GoToStep(0)
	, StartingStep(0)
	, TransitionToPreviousStartingStep(0)
	, GoBackToOriginStepUponSuccess(true)
	, ARTIFICIAL_BRANCH_ID_PREFIX("WorkflowArtificialBranchId_")
	, Verbose(false)
...
void osgi::core::WorkflowPrivate::onEntryInternal(
    WorkflowStep* step,
    WorkflowStep* comingFrom,
...
void osgi::core::WorkflowPrivate::onExitInternal(
    WorkflowStep* step,
...
void osgi::core::WorkflowPrivate::processingAfterOnExit()
{
	...
	else
    {
		this->OriginStep = 0;
		this->DestinationStep = 0;
		// we've exited the CurrentStep and haven't gone into another step, so we no longer have a currentStep.
		this->CurrentStep = 0;
    }
}
...
bool osgi::core::WorkflowPrivate::pathExists(const QString& goalId, WorkflowStep* origin)const
{
	...
	return (q->hasStep(goalId) && ((QString::compare(goalId, originId, Qt::CaseInsensitive) == 0) || (q->canGoForward(origin)))); //! <-- TODO insert logic here for looking at graph
}
